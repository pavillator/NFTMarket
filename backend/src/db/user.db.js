...
exports.create = (name, address, nonce, callback) => {
	var sql = "INSERT INTO users ( name, address, nonce ) VALUES ('"+name+"', '"+address+"', '"+nonce+"' )";
...
exports.update = (user, callback) => {
	pool.query(`UPDATE users SET name = '${user.name}', address = '${user.address}', nonce = '${user.nonce}' WHERE id = ${user.id}`, callback);
}
